...
#     print(f"Error loading model artifacts: {e}. Please ensure train_model.py has been run and paths in config.py are correct.")
#     print("Prediction functionality will be limited.")
...
#     return EMOTION_ALIASES.get(e, e)
...
#     if matches.empty:
#         return []
#     return matches.sample(n=min(limit, len(matches))).to_dict(orient="records")
...
# @app.route("/feeling") # Renamed from /stats to /feeling
# def feeling_page():
#     return render_template("feeling.html") # Renders the renamed template
...
#         log_to_db(user_input, emotion)
#         return jsonify({"emotion": emotion, "songs": songs})
#     return jsonify({"error": "No text provided"}), 400

# @app.route("/chart") # New route for the chart page
# def chart_page():
...
# import pathlib
# import pymysql
# from config import MODEL_PATHS # Assuming config.py has MODEL_PATHS defined
# import json
...
#     if matches.empty:
#         return []
#     # This will return all columns, including 'link' if it exists in songs_df
#     return matches.sample(n=min(limit, len(matches))).to_dict(orient="records")
...
# with MODEL_PATHS defined, e.g.:
# MODEL_PATHS = {
#     "model": "models/emotion_model.pkl",
#     "label_encoder": "models/label_encoder.pkl"
# }
from config import MODEL_PATHS
import json # Not strictly necessary for this app, but good practice for JSON handling
